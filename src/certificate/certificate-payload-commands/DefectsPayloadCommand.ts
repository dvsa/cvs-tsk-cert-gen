...
@Service()
export class DefectsPayloadCommand {
  protected type?: CERTIFICATE_DATA;

  protected isWelsh: boolean = false;

  constructor(private defectService: DefectService, private defectRepository: DefectRepository) {
  }
  ...
  private async getPayloadData(testResult: ITestResult, type: CERTIFICATE_DATA): Promise<any> {
    ...
    const defects = await this.generateDefects(testResult.testTypes, type, testResult.vehicleType, flattenedDefects, this.isWelsh);
    return defects;
  }
   ...
   */
  private generateDefects(testTypes: ITestType, type: CERTIFICATE_DATA, vehicleType: string, flattenedDefects: IFlatDefect[], isWelsh: boolean = false) {
    const defects = {
      DangerousDefects: [],
      MajorDefects: [],
      PRSDefects: [],
    ...
    testTypes.defects.forEach((defect) => {
      switch (defect.deficiencyCategory.toLowerCase()) {
        case 'dangerous':
          this.defectService.generateDangerousDefects(testTypes.testResult, defect, type, defects, vehicleType, isWelsh, flattenedDefects);
          break;
        case 'major':
          this.defectService.generateMajorDefects(testTypes.testResult, defect, type, defects, vehicleType, isWelsh, flattenedDefects);
          break;
        case 'minor':
          this.defectService.generateMinorDefects(defects, defect, vehicleType, testTypes.testResult, isWelsh, flattenedDefects);
          break;
        case 'advisory':
