import { InvokeCommandInput, InvokeCommandOutput, ServiceException } from "@aws-sdk/client-lambda";
...
class CertificateGenerationService {
  ...
  public async generateCertificate(
    ...
    return this.lambdaClient
      .invoke(invokeParams)
      .then(
        async (response: InvokeCommandOutput) => {
  ...
  public async getTestStations(): Promise<ITestStation[]> {
    ...
    while (retries < 3) {
      try {
        const response: InvokeCommandOutput = await this.lambdaClient.invoke(invokeParams);
  ...
  public async getTrailerRegistrationObject(vin: string, make: string) {
    ...
    try {
      if (!response.Payload || response.Payload as unknown as string === "") {
        throw new HTTPError(
          500,
