...
describe("Invoke certGen Function", () => {
    context("when the certGen function is invoked with valid test result", () => {
        ...
        it("should invoke certificate generate and upload services once", () => {
            ...
            let certGenServiceStub = sinon.stub(CertificateGenerationService.prototype, 'generateCertificate').resolvesThis();
            // Stub CertificateUploadService uploadCertificate method and resolve it
            let certUploadServiceStub = sinon.stub(CertificateUploadService.prototype, 'uploadCertificate').resolvesThis();
    ...
    context("when the certGen function is invoked with retroKey flag is set to true at test level in test results", () => {
        ...
        it("should not invoke certificate generate and upload services", () => {
            ...
            let certGenServiceStub = sinon.stub(CertificateGenerationService.prototype, 'generateCertificate');
            // Stub CertificateUploadService uploadCertificate method
            let certUploadServiceStub = sinon.stub(CertificateUploadService.prototype, 'uploadCertificate');
    ...
    context("when the certGen function is invoked with cvsTestUpdated flag is set to false at test-type level in test results", () => {
        const lambda = lambdaTester(certGen);
        const payload: any =
            {
                Records: [{
    ...
    context("when the certGen function is invoked with invalid testResultId for certificate generation.", () => {
        const lambda = lambdaTester(certGen);
        const payload: any =
            {
                Records: [{
