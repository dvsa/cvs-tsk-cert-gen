...
describe("cert-gen", () => {
    ...
    context("CertificateGenerationService", () => {
        ...
        context("CertGenService for Roadworthiness test", () => {
            context("when a passing test result for Roadworthiness test for TRL is read from the queue", () => {
                ...
                it("should return dgvw as 'grossDesignWeight' and weight2 as sum of 'designWeight' of the axles", () => {
                     ...
                     expect.assertions(1);
                     certificateGenerationService.getWeightDetails(testResult).then((weightDetails) => {
        ...
        context("CertGenService for Roadworthiness test", () => {
        context("when a passing test result for Roadworthiness test for HGV is read from the queue", () => {
            const event: any = cloneDeep(queueEventPass);
        ...
        it("should throw error", () => {
             ...
             expect.assertions(1);
             const expectedError = new HTTPError(500, "No vehicle found for Roadworthiness test certificate!");
             certificateGenerationService.getWeightDetails(testResult).catch((err) => {
        ...
        context("when a passing test result for Roadworthiness test for TRL is read from the queue", () => {
            ...
            testResult.vin = "T12768594";
            testResult.trailerId = "0285678";
            context("and weightDetails are fetched but not axles array is found", () => {
            it("it should throw error", () => {
                 ...
                 expect.assertions(1);
                 const expectedError = new HTTPError(500, "No axle weights for Roadworthiness test certificates!");
                 certificateGenerationService.getWeightDetails(testResult).catch((err) => {
                expect(err).toEqual(expectedError);
                getTechRecordStub.restore();
                });
